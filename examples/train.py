...
compiles_model      = CONFIG.MODEL.COMPILES
...
cxi_manager = CXIManager(path_csv = path_dataset)
idx_list    = cxi_manager.initialize_dataset(num_samples = size_sample)    # Already randomized
idx_train   , idx_val_and_test = split_dataset(idx_list        , frac_train   )
idx_validate, idx_test         = split_dataset(idx_val_and_test, frac_validate)
...
dataset_train = CXIDataset( idx_list       = idx_train,
...
dataset_validate = CXIDataset( idx_list       = idx_validate,
                               trans_list     = trans_list,
                               normalizes_img = True, )
