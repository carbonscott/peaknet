...
from .hiera_layers import DropPath, Mlp
...
class MaskUnitAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Reshape QKV output and split into Q, K, V for multi-head attention
        qkv = rearrange(
            self.qkv(x),
            'b (n_win n_tok) (three heads d) -> three b heads n_win n_tok d',
            n_win=num_windows,
            three=3,
            heads=self.heads,
            d=self.head_dim
        ...
        if self.q_stride > 1:
            ...
            # Group tokens by stride for pooling, then take max across stride dimension
            q = rearrange(
                q, 
                'b heads n_win (stride n_tok) d -> b heads n_win stride n_tok d',
        ...
        x = rearrange(x, 'b heads n_win n_tok d -> b (n_win n_tok) (heads d)')
        x = self.proj(x)
        return x
...
class Hiera(nn.Module):
    ...
    def get_pos_embed(self) -> torch.Tensor:
        if self.sep_pos_embed:
            # Repeat spatial embeddings across temporal dimension
            spatial = repeat(
                self.pos_embed_spatial, 
                '1 spatial_hw d -> 1 (t spatial_hw) d', 
                t=self.tokens_spatial_shape[0]
            )
            # Repeat temporal embeddings across spatial dimensions
            temporal = repeat(
                self.pos_embed_temporal, 
                '1 t d -> 1 (t spatial_hw) d', 
                spatial_hw=self.tokens_spatial_shape[1] * self.tokens_spatial_shape[2]
            )
    ...
    def forward(
        ...
        x = self.patch_embed(
            x,
            mask=(rearrange(mask, 'b (h w) -> b 1 h w', h=self.mask_spatial_shape[0]) 
                 if len(self.mask_spatial_shape) == 2 else 
                 rearrange(mask, 'b (t h w) -> b 1 t h w', t=self.mask_spatial_shape[0], h=self.mask_spatial_shape[1]))
        ...
        if mask is not None:
            # Expand mask to match token dimensions and apply boolean indexing
            mask_expanded = repeat(mask, 'b n -> b (n mu_size) d', mu_size=self.mu_size, d=x.shape[2])
            x = x[mask_expanded].view(x.shape[0], -1, x.shape[-1])
