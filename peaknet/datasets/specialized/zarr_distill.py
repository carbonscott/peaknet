...
class SegmentedPeakNetDataset(Dataset):
    ...
    def _item_generator(self):
        for file_idx, file_path in enumerate(self.file_paths):
            try:
                ...
                num_items = data.get('inputs').shape[0]
    ...
    def __getitem__(self, idx):
        ...
        image = torch.from_numpy(data.get("inputs")[item_idx][None,])
        label = torch.from_numpy(data.get("features")[item_idx][None,])

        # Apply transformation to image and label at the same time...
        if self.transforms is not None:
            data = torch.cat([image, *label.split(1,1)], dim = 0)    # (1,1,H,W), (1,1,H,W)
            ...
            label = torch.cat(data[1:].split(1,0),dim=1) # (1, 2, H, W)
